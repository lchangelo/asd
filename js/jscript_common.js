/*
	...
	$RCSfile: jscript_common.js,v $
	$Revision: 1.1.1.1 $
	$Date: 2008/09/14 10:28:14 $
*/
...
var BuyNowProduct={
	...
	try{
		if(!o) return;
		var product=document.getElementById("Checkout");
		if(!product){product=document.body.appendChild(document.createElement("DIV"));product.id="Checkout";}
		var t=o,lx=0,ly=0;if(!product)return;
		...
		product.style.width=mw+"px";
		product.style.height=mh+"px";
		product.style.left=parseInt(lx-(mw-o.offsetWidth)/2)+"px";
		product.style.top=parseInt(ly-(mh-o.offsetHeight)/2)+"px";
		if(F.ie)product.filters.Alpha.opacity=100; else product.style.MozOpacity=1;
		this.cfg={fly:product,source:o,x:lx,y:ly,"a":a,"b":b,w:o.offsetWidth,h:o.offsetHeight,time:setTimeout("BuyNowProduct.fly()",10)};
		product.style.display="block";
	}catch(e){}
	},
	fly:function(){
	try{
		var product=this.cfg.fly;if(!product)return;
		clearTimeout(this.cfg.time);
		if(product.offsetWidth>this.cfg.w+2||product.offsetHeight>this.cfg.h+2){
			product.style.width=parseInt(product.offsetWidth-this.cfg.a>this.cfg.w?product.offsetWidth-this.cfg.a:this.cfg.w-2)+"px";
			product.style.height=parseInt(product.offsetHeight-this.cfg.b>this.cfg.h-2?product.offsetHeight-this.cfg.b:this.cfg.h-2)+"px";
			product.style.left=parseInt(this.cfg.x-(product.offsetWidth-2-this.cfg.w)/2)+"px";
			product.style.top=parseInt(this.cfg.y-(product.offsetHeight-2-this.cfg.h)/2)+"px";
			return this.cfg.time=setTimeout("BuyNowProduct.fly()",10);
		}
		if(F.ie&&product.filters.Alpha.opacity>0){product.filters.Alpha.opacity-=5;this.cfg.time=setTimeout("BuyNowProduct.fly()",10);}
		else if(!F.ie&&product.style.MozOpacity>0){product.style.MozOpacity-=0.05;this.cfg.time=setTimeout("BuyNowProduct.fly()",10);}
		else{product.style.display="none";product.style.left=product.style.top="0px";}
	}catch(e){}
	}
};
function buyproduct(o){
	if(o){document.body.focus();BuyNowProduct.set(tags.buynowproduct);}
}
...
function serverList(gameid){
        ajaxresponse("server", "action=powerlevelingserver&gameid="+gameid);
	document.getElementById("hour").innerHTML = '';
        ajaxresponse("cleveling", "action=pwselectlvl&id=clevel"+"&gameid="+gameid);
        ajaxresponse("dleveling", "action=pwselectlvl&id=dlevel"+"&gameid="+gameid);
        ajaxresponse("class", "action=pwselectclass&gameid="+gameid);
	document.getElementById("Serverchoosegamenotice").style.display="none";
	document.getElementById("Clvlchoosegamenotice").style.display="none";
	document.getElementById("Dlvlchoosegamenotice").style.display="none";
	document.getElementById("Classchoosegamenotice").style.display="none";
	document.getElementById("NoticeChooseDesiredLevel").style.display="block";
}
function level() {
	ajaxresponse("hour", "action=powerlevelinghour&gameid="+document.getElementById("powerlevelinggame").value+"&current_lvl="+document.getElementById("clevel").value+"&desired_lvl="+document.getElementById("dlevel").value+"&servername="+document.getElementById("serverid").value+"&characterclass"+document.getElementById("characterclass").value);
}

function changecharact() {
	if(document.getElementById("powerlevelinggame").value!= '')
		level();
}

function NoticeSelectLvl(){
	alert("Please Choose Your Game And Desired Level.");
}
